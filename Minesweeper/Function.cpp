...
bool loadmedia()
{
	...
	if (!background.loadFromFile("res/images/back.jpg"))
	{
		success = false;
	}
	//Open the font
	gFont = TTF_OpenFont("res/font.ttf", 30);
	if (gFont == NULL)
	{
...
bool loadMenuMedia()
{
	...
	if (!menuTheme.loadFromFile("res/images/minesweeper.png"))
	{
		cout << "Fail";
        success = false;
	}
	...
	if (!customStart.loadFromFile("res/images/mode.jpg"))
	{
		cout << "Fail";
		success = true;
	}
...
void CustomMode()
{
	...
	SDL_SetWindowSize(window, 889, 500);
	...
	bool play = false;
	SDL_Color textcolor = { 255,0,0 };
    menu5Color.loadFromRenderedText("PLAY", textcolor);
	while (customMode)
	{
		SDL_StartTextInput();
		while (SDL_PollEvent(&e) != 0)
		{
            ...
            case SDL_MOUSEBUTTONDOWN:
				if (x > 375 && x < 450 && y>90 && y < 118) widthInside = true;
				else widthInside = false;
				if (x > 375 && x < 450 && y>190 && y < 218) heightInside = true;
				else heightInside = false;
				if (x > 375 && x < 450 && y>290 && y < 318) mineInside = true;
				else mineInside = false;
				if (x > 400 && x < 500 && y>375 && y < 403) play = true;
				else play = false;
				break;
            case SDL_MOUSEMOTION:
                if (x > 400 && x < 500 && y>375 && y < 403){
                    menu5Color.render(400, 375);
				}else{
                    menu5.render(400, 375);
				}
			case SDL_TEXTINPUT:
        ...
        background.render(375, 90);
        background.render(375, 190);
        background.render(375, 290);
		wInput.render(374, 90);
		hInput.render(374, 190);
		mInput.render(374, 290);

		SDL_Color textcolor1 = { 255,255,255 };
        if (!menu2.loadFromRenderedText("WIDTH", textcolor1))
        {
            cout << "fail!";
        }
        if (!menu3.loadFromRenderedText("HEIGHT", textcolor1))
        {
            cout << "fail!";
        }
        if (!menu4.loadFromRenderedText("MINES", textcolor1))
        {
            cout << "fail!";
        }
        if (!menu5.loadFromRenderedText("PLAY", textcolor1))
        {
            cout << "fail!";
        }
        menu2.render(201, 90);
        menu3.render(201, 190);
        menu4.render(201, 290);
        //menu5.render(400, 375);
		if (play == true)
		{
			isRunning = true;
